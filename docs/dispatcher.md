...
The main-thread requirement also affects the macOS MSAL broker. MSAL decides
**once per process** whether an `NSApplication` is running, and caches that
answer. If one is not running it requires interactive broker calls to be made
on thread 1, and then takes that thread over with its own polling loop - which
cannot coexist with a UI main loop. If one *is* running it requires neither.
...
`_outstandingJobs`|Every accepted job, tracked until its completion task settles, including time spent awaiting asynchronous work. This is what makes a main loop failure recoverable - see [Failure handling](#failure-handling).
...
Each job owns its work delegate and completion source. `DispatcherJob<T>`
handles synchronous work; `AsyncDispatcherJob` and `AsyncDispatcherJob<T>`
handle task-returning work. Their shared `DispatcherJob` base restores the
execution context, applies Trace2 attribution, and catches synchronous
exceptions from invoking the delegate.
...
work *starts* rather than when it finishes. These overloads forward the work's
task outcome into a completion source owned by the dispatcher. The task
returned to the caller is that completion source's task, not an `Unwrap()`
proxy, so `RunContinuationsAsynchronously` applies to the caller's
continuations as well. Results, exceptions, and cancellation are preserved.

Each async job's `CompleteAsync` helper awaits the work's task without
capturing the main loop's synchronization context. `SuppressThrowing` lets it
observe completion before forwarding the original outcome with
`TrySetFromTask`, including all exceptions and the cancellation token.
Failures are transferred to the caller's task, not discarded.

The queue tracks `IDispatcherJob.Completion`, without needing to distinguish
sync and async jobs. An asynchronous job remains in `_outstandingJobs` after
its first yield, so a later main-loop failure can still fault the caller's
task even when the work's own continuation can no longer run. Removal from
the set does not depend on the main loop being available. Cleanup observes
the registered job's completion, including faults, without rethrowing or
reporting them; the caller's task keeps its original outcome.
...
> the main loop can accept work; only waiting for the work's completion is
> omitted.
...
**Outstanding work is abandoned, not drained.** Tasks that still depend on the
stopped loop may never complete. Already-started asynchronous work can still
settle independently, for example after `ConfigureAwait(false)` or cooperative
cancellation. `Shutdown()` must therefore be called only once the application
has finished everything it cares about. Draining would hang the common case
of a window shown without anyone awaiting it - there would be nothing left to
close the window, and so nothing to wait for.
...
- **Do not synchronously block on a dispatcher task from inside a job.**
  `Wait()`, `.Result`, and `GetAwaiter().GetResult()` can block the loop needed
  to finish that work. Asynchronously awaiting nested dispatcher work is
  supported; it yields the loop while the task is pending.
...
rendezvous between a posting caller and the starting main loop testable. Its
synchronization context queues asynchronous continuations back to the fake
loop, so failures after a job yields can be exercised too. Tests cover signals
preceding waits, concurrent posters, asynchronous completion, and shutdown
while initialization is still blocked, whether it later succeeds or fails. See
